...
class ReviewController {
  ...
  public static async addReview(
    ...
    const cloudinaryResult = (req as any).cloudinaryResult || { secure_url: '' }; 
...
// edit review
  public static async editReview(
    ...
    try {
      ...
      if (
        rating !== undefined &&
        (typeof rating !== "string" || isNaN(Number(rating)) || Number(rating) < 1 || Number(rating) > 5)
      ) {
        res.status(400).json({ 
          message: "Rating must be a number between 1 and 5",
          field: "rating",
      ...
      if (
        message !== undefined &&
        (typeof message !== "string" || message.length < 5 || message.length > 100 || message.trim() === "")
      ) {
        res.status(400).json({ 
          message: "Message must be a string between 5 and 100 characters",
          field: "message",
      ...
      if (review.userId !== userId) {
        ...
        return;
            ...
            if (cloudinaryResult && cloudinaryResult.secure_url) {
              ...
              cloudinary.uploader.destroy(oldReviewImage, (error: any, result: any) => {
                if (error) {
