...
const Product = () => {
  ...
  const allBrands = [...new Set(products.map(p => p.brand || 'Unknown'))].sort();
  ...
  const filteredProducts = products
    .filter((p) => {
      ...
      const matchCategory = categoryFilter === 'all' || p.Category?.categoryName === categoryFilter;
      const matchPrice = p.productPrice >= priceRange[0] && p.productPrice <= priceRange[1];
      const matchRating = ratingFilter === 0 || (p.Ratings || 0) >= ratingFilter;
    ...
    .sort((a, b) => {
      ...
      if (sortBy === 'name-a-z') return a.productName.localeCompare(b.productName);
      if (sortBy === 'name-z-a') return b.productName.localeCompare(a.productName);
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {currentItems.length > 0 ? (
            ...
            <p className="col-span-full py-12 text-center text-lg text-gray-500">
          ...
          <div className="mb-4">
            ...
            <div className="max-h-64 overflow-y-auto space-y-3 pr-2">
              {filteredBrands.map((brand) => (
                <label key={brand} className="flex items-center gap-3 cursor-pointer hover:bg-indigo-50 p-2 rounded-lg transition">
                  <input
                    type="checkbox"
              ...
              {filteredBrands.length === 0 && (
                <p className="text-center text-sm text-gray-500 py-4">No brands match your search</p>
              )}
            </div>
          </div>
